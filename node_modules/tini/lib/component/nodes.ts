...
import {Registry} from '../directives/registry';
...
const propagate = (binders: Binders) => (key: string) => {
    Object.keys(binders).forEach(path => {
        key === '*' || path.startsWith('this.' + key) ? binders[path].forEach(fn => {
            try {
                return fn();
            } catch (e) {
                return undefined;
...
export const scanDOMTree = (root: Element, element: HTMLElement, localDirectives: Directive[] = []) => {
